...
`nop` instruction after every branch or jump instruction (including `jal` and `jr`).
The solution is identical for delayed loads, put a `nop` after every load instruction.
...
| Load Immediate | li   $t0, 42 | ori   $t0, $0, 42   # also addi  $t0, $0, 42

| Move           | move $t0, $t1 | or   $t0, $0, $t1  # also add   $t0, $0, $t1

| Load Address   | la   $t0, label | lui  $t0, 0x1000  # or 0x1001+
ori  $t0, $0, offset   # offset is byte offset from start of .data to label
