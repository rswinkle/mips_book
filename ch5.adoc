...
different operating systems on the same architecture but that's beyond the scope
of this book.

What matters, is that the calling convention works by setting rules (and guidelines)
for register use, and when/how to use the stack.
...
or pass more than 4 arguments twice.
